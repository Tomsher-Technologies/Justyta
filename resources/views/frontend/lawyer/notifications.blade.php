@extends('layouts.web_lawyer', ['title' => __('frontend.notifications')])

@section('content')
    <div class="container">
        <h2 class="text-xl font-bold mb-4">{{ __('frontend.all_notifications') }}</h2>
        <div class="bg-white rounded-lg p-6">
            <div class="flex items-start xl:items-center justify-between flex-col x:flex-row">
    ...
    <script>
        ...
        document.getElementById('delete-all').addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch("{{ route('lawyer.notifications.clear') }}", {
                            method: "POST",
                            headers: {
                                'X-CSRF-TOKEN': "{{ csrf_token() }}",
                                'Content-Type': 'application/json'
