    ...
    <div class="bg-white rounded-lg p-6">
        <div class="flex items-center justify-between">
            ...
            <div class="text-end mt-4 flex items-center gap-4">
                ...
                <button class="bg-red-500/20 text-red-500 p-4 py-1.5 rounded-md text-sm" id="delete-all">{{ __('frontend.clear_all') }}</button>
            </div>
        </div>
                ...
                <thead class="text-md font-normal">
                    <tr class="bg-[#07683B] text-white font-normal">
                        <th scope="col" class="px-6 py-5 font-semibold text-start">
                            <input type="checkbox" id="select-all" class="w-4 h-4 text-[#cdb57a] bg-gray-100 border-gray-300 rounded-sm focus:ring-[#cdb57a] focus:ring-2">
                        </th>
                ...
                <tbody>
                    @forelse($notifications as $key => $notification)
                        <tr class="border-b text-[#4D4D4D]">
                            <td class="px-6 py-4">
                                <input type="checkbox" class="notif-checkbox w-4 h-4 text-[#cdb57a] bg-gray-100 border-gray-300 rounded-sm focus:ring-[#cdb57a] focus:ring-2" value="{{ $notification['id'] }}">
                            </td>
                            <td class="px-6 py-4">
                    ...
                    @empty
                        <tr><td colspan="4" class="items-center text-center p-12">{{ __('frontend.no_details_found') }}</td></tr>
                    @endforelse
                </tbody>
            </table>
                ...
                if (result.isConfirmed) {
                    ...
                    .then(data => {
                        if (data.success) {
                            ...
                            toastr.error('Error: ' + (data.message || '{{ __("frontend.unable_delete_notifications") }}'));
                        }
                    })
                    .catch(err => {
