...
@section('content')
    <div class="bg-white rounded-lg p-6">
        ...
        <form class="" autocomplete="off" action="{{ route('jobs.store') }}"
            ...
            <div class="grid grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <input type="date" name="deadline_date"
                        class="w-full border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500"
                        value="{{ old('deadline_date') }}" min="{{ \Carbon\Carbon::now()->format('Y-m-d') }}">
                    @error('deadline_date')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="col-span-1 md:col-span-4 mt-6" x-data="{ activeTab: '{{ $languages->first()->code }}' }">
                ...
                <!-- Tabs -->
                <div class="flex flex-wrap border-b border-gray-400 bg-gray-200 rounded-t-lg">
                    @foreach ($languages as $lang)
                        <button type="button" @click="activeTab = '{{ $lang->code }}'"
                            :class="activeTab === '{{ $lang->code }}'
                                ?
                                'text-indigo-700 font-medium border-b-4 border-indigo-500 bg-white' :
                                'text-gray-700 font-medium hover:text-indigo-500 hover:bg-gray-100'"
