        ...
        <form method="POST" action="{{ route('vendor.service.legal-translation-request') }}" id="legalTranslationForm" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-1 gap-6">
                <div class="lg:col-span-2 bg-white p-6 xl:p-10 rounded-[20px] border !border-[#FFE9B1]">
                    ...
                    <div class="mt-6">
                        <div class="text-gray-700 text-lg mb-2 ">{{ __('frontend.completion_hours') }} <span class="font-semibold text-xl text-[#07683B]" id="translation_completion_hours">0</span></div>
                        <div class="text-gray-700 text-lg mb-2 ">{{ __('frontend.payment_amount') }} <span class="font-semibold text-xl text-[#07683B]" id="translation_price_result">{{ __('frontend.AED') }} 0.00</span></div>
                        
                        <div class="flex items-center gap-4">
                            <button type="submit" id="submit_button" class="bg-[#07683B] text-white px-4 py-2 rounded-md shadow hover:bg-[#07683B] w-[200px]">
                                {{ __('frontend.submit') }}
                            </button>

                            <a href="{{ route('vendor.translation-requests') }}" class="bg-[#cdb57b] text-black px-4 py-2 border-2 rounded-md shadow hover:bg-[#ffffff] w-[200px] text-center">
                                    {{ __('frontend.cancel') }}
                            </a>
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#document_type').on('change', function () {
                ...
                if (documentTypeId) {
                    $.ajax({
                        url: '{{ route("vendor.get.sub.document.types") }}',
                        type: 'POST',
                        data: {
            ...
            function calculateTranslationPrice() {
                ...
                if (from_language_id && to_language_id && no_of_pages && doc_type && doc_sub_type && receive_by) {
                    $.ajax({
                        url: "{{ route('vendor.calculate-translation-price') }}", 
                        type: 'POST',
                        data: {
