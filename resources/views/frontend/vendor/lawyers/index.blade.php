        ...
        <div class="grid grid-cols-1 md:grid-cols-12 items-end gap-4 mb-8">
            ...
            <div class="col-span-2">
                ...
                <select name="status"
                    ...
                    <option value="1" {{ request()->status == 1 ? 'selected' : '' }}>{{ __('frontend.active') }}
                    </option>
                    <option value="2" {{ request()->status == 2 ? 'selected' : '' }}>{{ __('frontend.inactive') }}
                    </option>
                </select>
            </div>
            ...
            <div class="col-span-4">
                <label for="countries" class="block mb-2 text-sm font-medium text-gray-900">{{
                    __('frontend.specialities') }}</label>
                <select name="specialities" id="select-tag"
            ...
            <a href="{{ route('vendor.edit.lawyers', base64_encode($lawyer->id)) }}"
                ...
                <svg class="w-6 h-6 text-gray-700" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                    ...
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M09 7H4a2 3 0 00-2 2v12a2 2 0 002 2h12a2 2 0 002-2v-06M16.5 3.5a2.121 2.1 0 013 3L10 19l-4 1 1-4L16.5 3.5z">
                    </path>
                </svg>
            </a>
            ...
            <a href="{{ route('vendor.view.lawyers', base64_encode($lawyer->id)) }}">
                <div class="flex flex-col xl:flex-row items-middle gap-6 w-full">
                    <div class="relative inline-block">
                        ...
                        <span class="absolute bottom-2 right-2 w-5 h-5 bg-green-500 border-2 border-white rounded-full"
                            title="Online"></span>
                        @else
                        <span class="absolute bottom-2 right-2 w-5 h-5 bg-gray-400 border-2 border-white rounded-full"
                            title="Offline"></span>
                        @endif

                        <span id="online-status-{{ $lawyer->id }}"
                            class="absolute bottom-2 right-2 w-5 h-5 rounded-full border-2 border-white {{ $lawyer->user->is_online ? 'bg-green-500' : 'bg-gray-400' }}"
                            title="{{ $lawyer->user->is_online ? 'Online' : 'Offline' }}"></span>

                    </div>
                    <div>
                        <div class="border-b pb-4 mb-4">
                            <h3 class="text-lg font-semibold text-gray-900">{{ $lawyer->getTranslation('full_name',
                                app()->getLocale()) }}</h3>
                            <p class="text-sm text-gray-500">{{ $lawyer->ref_no }}</p>
                        </div>
                        <div class="text-sm text-gray-700">
                            <p>{{ __('frontend.last_login') }} : <span class="font-medium">
                                    {{ ($lawyer->user?->last_login_at != null) ? date('d M Y, h:i A',
                                    strtotime($lawyer->user?->last_login_at)) : '' }}
                                </span></p>
                            <p>{{ __('frontend.no_of_consultation') }} : <span class="font-medium">{{
                                    lawyerTotalAcceptedConsultations($lawyer->id) }}</span></p>
                        </div>
                    </div>
                </div>
            </a>
