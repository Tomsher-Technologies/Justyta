    <section class="bg-[#fff] px-5 md:px-10 lg:px-10 py-[80px] relative">
        <div class="bg-[#FFF9F4] p-6 xl:p-12 h-full ">
            <div class="w-[100%] xl:w-[60%]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-2 gap-5 xl:gap-12 mt-8 xl:w-max">
                    @if(!empty($items) && is_array($items) && count($items) > 0)
                    @foreach($items as $item)
                    <div class="border-b !border-[#C9C5C1] flex flex-col xl:flex-row items-start xl:items-center gap-5 pb-6">
                        <h3 class="font-cinzel text-[30px] font-bold text-[#B9A572] basis-auto xl:basis-[120px]"> {!! $item['title'] !!}</h3>
